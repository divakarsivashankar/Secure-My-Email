...
util::Status OAuth2AuthorizationFlow::SimpleJsonData::Init(const string& json) {
    Json::Reader reader;
    Json::Value myroot;
    std::ifstream stream(json, std::ifstream::binary);
 // std::istringstream stream(json);
  if (!reader.parse(stream, myroot)) {
...
string OAuth2AuthorizationFlow::SimpleJsonData::InitFromContainer(
  ...
  if (!OAuth2AuthorizationFlow::SimpleJsonData::Init(json).ok() || json_.begin() == json_.end()) {
    return "";
  }
...
bool OAuth2AuthorizationFlow::GetStringAttribute(
  ...
  return data->GetString(key, value);
    ...
    const char googleapis::client::OAuth2AuthorizationFlow::kOutOfBandUrl[] =
...
OAuth2AuthorizationFlow*
    googleapis::client::OAuth2AuthorizationFlow::MakeFlowFromClientSecretsJson(
    const string& json,
